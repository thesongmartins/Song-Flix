...
const MovieDetails = () => {
  ...
  if (isError)
    content = <p>{`Ooops! seems like we encountered an error: ${error}`}</p>;
  ...
  if (movie) {
    ...
    //Movie contents and cards
    content = (
      <section key={movie.id}>
        <section className="w-full flex flex-col gap-4 lg:py-6">
          <div
            className="rounded-lg h-[60vh] p-2 w-full lg:w-full px-4"
            key={movie.id}
          >
            <iframe
              className="w-full rounded-lg h-full lg:w-full mx-auto"
              src={`https://www.youtube.com/embed/${trailer[0].key}`}
            ></iframe>
          </div>
          <article className="grid lg:flex gap-2 px-4">
            <div
              className="lg:w-[60%] my-4 pr-6 text-gray-700 flex flex-col gap-2"
              key={movie.id}
            >
              <ul className="flex justify-between items-center mb-4 flex-wrap gap-2">
                <li
                  className="text-[#111827] font-semibold text-xl"
                ...
                <li className="text-sm" data-testid="movie-runtime">
                  {movie.runtime}m
                </li>
                <li className="flex gap-2">
                  {movie.genres.map((genre) => (
                    <span className="text-[#be123c] text-sm border border-[#be123c] p-[2px] rounded-lg">
                      {genre.name}
                    </span>
                  ))}
                </li>
              ...
              <ul className="flex flex-col gap-2 lg:gap-6">
                ...
                <li>
                  ...
                  {writers.slice(0, 2).map((writer) => (
                    <span className="text-[#be123c]" key={writer.name}>
                      {" "}
                      {writer.name},{" "}
                    </span>
                  ))}
                ...
                <li>
                  Stars:
                  {stars.map((star) => (
                    <span className="text-[#be123c]" key={star.name}>
                      {" "}
                      {star.name},{" "}
                    </span>
                  ))}
